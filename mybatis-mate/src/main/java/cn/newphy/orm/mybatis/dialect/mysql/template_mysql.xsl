...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    ...
    <mapper namespace="{$namespace}">
      ...
      <delete id="batchDelete" parameterType="java.util.Collection">
        ...
        <where>
          ...
          <foreach item="entity" index="index" collection="list" open="(" separator="," close=") ">
            #{entity.<xsl:value-of select="$idColumn/property"/>}
          </foreach>
        </where>
      </delete>
      ...
      <select id="listBy" resultMap="ResultMap">
        SELECT
        <include refid="sqlColumns"/>
        FROM
      ...
      <sql id="sqlCondition">
        ...
        <xsl:for-each select="mybatisMapping/propertyMappings/propertyMapping">
          <if>
            <xsl:attribute name="test"><xsl:value-of select="property"/> != null</xsl:attribute>
            AND <xsl:value-of select="column"/>=#{<xsl:value-of select="property"/>}
          </if>
        </xsl:for-each>
        </where>
      </sql>
      ...
      <sql id="sqlOrderBy">
        <if>
        <xsl:attribute name="test">__orders != null</xsl:attribute>
