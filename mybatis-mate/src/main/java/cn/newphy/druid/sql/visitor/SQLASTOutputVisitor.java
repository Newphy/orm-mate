...
import cn.newphy.druid.sql.ast.SQLOrderingSpecification;
...
import cn.newphy.druid.sql.ast.expr.SQLExprUtils;
...
import cn.newphy.druid.sql.ast.expr.SQLIntervalUnit;
...
import cn.newphy.druid.sql.ast.statement.SQLColumnConstraint;
...
import cn.newphy.druid.sql.ast.statement.SQLCreateTriggerStatement.TriggerType;
...
import cn.newphy.druid.sql.ast.statement.SQLForeignKeyImpl.Match;
...
import cn.newphy.druid.sql.ast.statement.SQLObjectType;
...
public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    ...
    static {
        try {
            defaultPrintStatementAfterSemi = Utils
                .getBoolean(System.getProperties(), "druid.sql.output.printStatementAfterSemi"); // compatible for early versions
        } catch (Throwable ex) {
            // skip
        }
    }
    ...
    public boolean visit(SQLColumnReference x) {
        ...
        Match match = x.getReferenceMatch();
        if (match != null) {
